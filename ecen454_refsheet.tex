...
\zzz{MOS transitive I-V Characteristics and Parasitics}
  ...
  \1 capacitor effect:
    \2 gate and channel can have a parallel plate capacitor effect, with the thin layer of SiO$_2$ acting as the insulator
    \2 $C=\frac{Q}{V}=\eta_{SiO_2}wl/t_{SiO_2}, V = V+{gc}-V_t=(v_{gs}-V{ds}/2)-V_t$
  ...
  \1 carrier velocity: velocity of the electrons?
    ...
    \\ $L$: length of channel
  \1 Shockley model of transistor: TODO
    \2 1st order model: $\beta=\mu C_{SiO_2}\frac{w}{l}$
    \\ $I_{ds}|V_{gs}<V_t=0$: cutoff
    \\ $I_{ds}|V_{ds}<V_{dstat}=\beta(V_{gs}-V_t-V_{ds}/2)V_{ds}$: linear
    \\ $I_{ds}|V_{ds}<V_{dstat}=\frac{\beta}{2}(V_{gs}-V_t)^2$: saturation
    \2 Must be able to derive this model on exam!
